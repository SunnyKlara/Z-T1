...
// ══════════════════════════════════════════════════════════════
// 职责：用户中心 — 分区标题 + icon tile 列表 + 连接状态展示
// 不做什么：不处理 BLE 连接逻辑（由 Provider 层处理）
...
class UserCenterScreen extends ConsumerWidget {
      ...
      body: ListView(
        ...
        children: [
          ...
          _tile(
            ...
            title: '当前设备',
            trailing: connectedDevice?.name ?? '未连接',
  ...
  Widget _sectionTitle(String text) {
    return Padding(
      ...
      child: Text(text, style: TextStyle(color: Colors.white.withAlpha(100), fontSize: 12, fontWeight: FontWeight.w500, letterSpacing: 1.0)),
    );
  }
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: Color(0x14FFFFFF), width: 0.5))),
          child: Row(children: [
